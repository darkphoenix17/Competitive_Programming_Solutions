...
int main()
{
    ...
    while (t--) {
        ...
        ll minicheck;
        ll rem = n % 4;
        if (rem == 0) minicheck = min(n, 4 * 1ll);
        else if (rem == 1) minicheck = min(n, 5 * 1ll);
        else if (rem == 2) minicheck = min(n, 6 * 1ll);
        else if (rem == 3) minicheck = min(n, 7 * 1ll);
        ll cur;
        loop(i, 2, minicheck) {
            ll temp = cur_max;
            temp = temp % 10;
