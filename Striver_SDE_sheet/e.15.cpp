...
bool solve(ll &x, ll &y, ll a[3]) {
	loop(i, 0, 3) {
		ll A = a[i], B = a[(i + 1) % 3], C = a[(i + 2) % 3];
		ll ny = y - (A + x - 1) / x;
		if (ny > 0 && (B + x - 1) / x + (C + x - 1) / x <= ny) return true;
		if (ny > 0 && (B + ny - 1) / ny + (C + ny - 1) / ny <= x) return true;
	}
	return false;
}
...
int main()
{
	...
	while (t--) {
		ll x, y, a[3];
		cin >> x >> y >> a[0] >> a[1] >> a[2];
		cout << ((solve(x, y, a) || solve(y, x, a)) ? "Yes" : "No") << endl;
	}
}
